...
const handleExportPDF = async (template: ContractTemplate, formData: Record<string, string>, clientName: string, plate: string, activeFont: string, isWhatsApp: boolean = false) => {
  ...
  // CRITICAL: Ensure fonts are 100% loaded to prevent text width calculations error (Right-Shift fix)
  ...
  for (let i = 0; i < pagesToProcess.length; i++) {
    ...
    activeFields.forEach(field => {
      ...
      // FIXING DOWN-SHIFT: Using fixed height and flex-center for absolute pivot
      fieldEl.style.height = '40px'; 
      ...
      // Internal Span to mimic Print Layout exactly
      const textSpan = document.createElement('span');
      textSpan.style.width = '100%';
      textSpan.style.textAlign = field.alignment === 'L' ? 'left' : field.alignment === 'R' ? 'right' : 'center'; 
      textSpan.style.lineHeight = '1';
      textSpan.innerText = formData[field.key] || '';
      
      // FIXING PIVOT ERROR: Fixed pixel shift translateY(-20px) instead of percent based translateY(-50%)
      fieldEl.style.transform = `translateY(-20px) rotate(${field.rotation}deg)`; 
      fieldEl.style.transformOrigin = 'center center';
